...
import { activatePlusSubscription } from '@/lib/subscription';
...
export async function POST(req: NextRequest) {
  ...
  const subscription =
    'isSubscription' in pkg && pkg.isSubscription
      ? await activatePlusSubscription(user.id)
      : null;

  return NextResponse.json({ success: true, credits: pkg.credits, subscription });
}
