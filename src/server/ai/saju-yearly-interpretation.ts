...
export const SAJU_YEARLY_INTERPRETATION_PROMPT_VERSION = 'saju-yearly-interpret-v1';
...
export function parseYearlyInterpretationText(
  ...
  try {
    ...
    const monthlyFlows = normalizeMonthlyFlows(parsed.monthlyFlows);
    ...
    if (
      ...
      !secondHalf ||
      !categories ||
      monthlyFlows.length < MIN_MONTHS ||
      ...
      return {
        ...
        errorMessage:
          'Yearly AI interpretation JSON is missing required sections or monthly flows.',
      };
    }
...
export function createYearlyInterpretationPrompt(
  ...
  return {
    instructions: [
      ...
      'monthlyFlows는 1월부터 12월까지 반드시 모두 채우고, 체감 가능한 변화 중심으로 씁니다.',
